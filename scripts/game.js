...
function showPipe(pipe) {
    $("#pipes").append("<div class='pipe' style='right:" + pipe.x + "px;height:" + pipe.height + "px;'>");
    $("#pipes").append("<div class='pipe' style='right:" + pipe.x + "px;height:" + (400 - pipe.height) + "px;top:0px;'>");
}
...
function checkBird() {
    ...
    if (temp != null) {
        if ((birdY < temp.height) || (birdY > (600 - (400 - temp.height)) - 20)) {
            clearInterval(gameTimer);
            gameEnded = true;
            $("#game").append("<div id='gameOver'><h1>Game Over</h1><form action=''><input id='button' type='submit' value='New Game'></form></div>");
